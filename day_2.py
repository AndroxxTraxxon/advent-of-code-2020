...
for line in data.splitlines():
    rule, passcode = map(lambda s: s.strip(), line.split(':'))
    qtyRange, letter = rule.split(' ')
    qtyRange = tuple(map(int, qtyRange.split('-')))
    items.append((*qtyRange, letter, passcode))

for item in items:
    low, high, letter, passcode = item
    if passcode.count(letter) in range(low, high + 1):
        passing.append(item)
    else:
        failing.append(item)
...
for item in items:
    low, high, letter, passcode = item
    if passcode[low-1] == letter:
        if passcode[high-1] == letter:
            failing.append(item)
        else:
            passing.append(item)
    elif passcode[high-1] == letter:
        passing.append(item)
    else:
        failing.append(item)
