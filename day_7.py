data = '''mirrored beige bags contain 1 drab brown bag, 3 dotted crimson bags.
...
bright red bags contain 1 pale beige bag.'''.splitlines()

target_name = 'shiny gold'

containment_map = dict()
for line in data:
    container, contents = line.split('bags contain')
    container_name = container.replace('bags', '').strip()
    if 'no other bags' in contents:
        contents = []
        # print('empty bag!')
    else:
        contents = list(map(lambda s: s.strip(), contents.strip().split(',')))
    content_tuples = list()
    for content in contents:
        words = content.split()
        qty = words[0]
        descriptor = ' '.join(words[1:-1])
        content_tuples.append((descriptor, int(qty)))
    containment_map[container_name] = content_tuples

import pprint
# pprint.pprint(containment_map)

containment_paths = dict()
has_new_info = True
round_no = 0
while has_new_info:
    try:
        print(f'\nRound {round_no}:')
        print(f'{len(containment_paths)} found paths')
        has_new_info = False
        for container, contents in containment_map.items():
            paths = list()
            if container.lower() == target_name and container.lower() not in containment_paths:
                paths.append(tuple())
            for name, qty in contents:
                if name.lower() in containment_paths:
                    # print(f'Processing {container}: Found {name} in existing paths')
                    for path in containment_paths[name.lower()]:
                        paths.append(((name, qty), *path))
            if len(paths) > 0 and str(containment_paths.get(container.lower())) != str(paths):
                containment_paths[container.lower()] = paths
                # print("Updated paths for: " + container)
                has_new_info = True
...
# pprint.pprint(set(containment_paths.keys()))
print(len(containment_paths)-1)

def count_bags(bag_name):
    _total = 1
    print(bag_name, )
    for name, qty in containment_map[bag_name]:
        _total += count_bags(name) * qty
    return _total

print(count_bags(target_name)-1)