...
cfmsluzogvixrdbetnq
'''.split('\n\n')
groups = list(map(lambda s: set(c for c in s.replace('\n', '') if c.isalpha()), data))
import pprint
pprint.pprint(groups)

print(sum(map(lambda s: len(s), groups)))

import string
universal_set = set(c for c in string.ascii_lowercase)

groups.clear()
for group in data:
    persons = list(set(c for c in person) for person in group.splitlines())
    groups.append(universal_set.intersection(*persons))
